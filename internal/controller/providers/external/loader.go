...
// injects the opaque config bag, and returns a providers.Provider adapter plus
...
func Load(ctx context.Context, cmd *exec.Cmd, config any) (providers.Provider, *pluginproto.Client, error) {
	if cmd.Stderr == nil {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("spawn plugin: %w", err)
	}

	hs, err := client.Handshake(ctx)
	if err != nil {
		_ = client.Close(ctx)
		return nil, nil, fmt.Errorf("plugin handshake: %w", err)
	}
	if hs.ProtocolVersion != pluginproto.ProtocolVersion {
		_ = client.Close(ctx)
		return nil, nil, fmt.Errorf("plugin protocol version %q, controller speaks %q",
			hs.ProtocolVersion, pluginproto.ProtocolVersion)
	}
	if hs.ProviderName == "" {
		_ = client.Close(ctx)
		return nil, nil, fmt.Errorf("plugin returned empty provider name")
	}

	if config != nil {
		if err := client.Configure(ctx, config); err != nil {
			_ = client.Close(ctx)
			return nil, nil, fmt.Errorf("configure plugin %q: %w", hs.ProviderName, err)
		}
	}

	return New(client, hs), client, nil
}
